...
print(f"Available Environment attributes: {[attr for attr in dir(Environment) if not attr.startswith('_')]}")
...
def fetch_klines(private_key, market, days, resolution, use_testnet=True):
    ...
    # Use correct environment names from paradex_py
    if use_testnet:
        env = Environment.TESTNET
        env_name = "TESTNET"
    else:
        env = Environment.PROD  # Try PROD first
        env_name = "MAINNET"
